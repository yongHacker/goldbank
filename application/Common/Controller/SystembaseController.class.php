...
class SystembaseController extends AppframeController {
	...
	function _initialize(){
       ...
       $session_admin_id=session('SYSTEM_ID');
    	if(!empty($session_admin_id)){
			...
			if(IS_AJAX){
				$this->error("您还没有登录！",U("system/public/login"));
			}else{
				//header("Location:".U("admin/public/login"));
				echo("<script>window.parent.location.href='".U("system/public/login")."'</script>");
				exit();
			}
	...
	private function check_access($uid){
		...
		$no_need_check_rules=array("AdminIndexindex","AdminMainindex","KunjinjubaoSettingmenu_notice","KunjinjubaoZclmzclm_data");

		if( !in_array($rule,$no_need_check_rules) ){
			return sp_auth_check($uid,null,"or","a");
		}else{
			return true;
		}
	}
    ...
    private function load_app_admin_menu_lang(){
		...
		if($default_lang!=$langSet){
		    $admin_menu_lang_file=SPAPP.MODULE_NAME."/Lang/".$langSet."/system_amenu.php";
		}else{
		    $admin_menu_lang_file=SITE_PATH."data/lang/".MODULE_NAME."/Lang/$langSet/system_amenu.php";
	...
	private function load_menu_lang(){
	    ...
	    foreach ($apps as $app){
	        if(is_dir(SPAPP.$app)){
	            if($default_lang!=$langSet){
	                $admin_menu_lang_file=SPAPP.$app."/Lang/".$langSet."/system_amenu.php";
	            }else{
	                $admin_menu_lang_file=SITE_PATH."data/lang/$app/Lang/".$langSet."/system_amenu.php";
	                if(!file_exists_case($admin_menu_lang_file)){
	                    $admin_menu_lang_file=SPAPP.$app."/Lang/".$langSet."/system_amenu.php";
	                }
	            }
	 ...
	 * 将所属表的所有参数输出
	 */
	protected function show_status($table){
		$status_list=D('System/AStatus')->getList("`table`='".M($table)->getTableName()."'");
		if(!empty($status_list)){
			foreach($status_list as $key=>$val){
				$s_id=$val['id'];
				$field=$val['field'];
				$tpl=array();
				$value_list=D("System/AStatusValue")->getList(array('s_id'=>$s_id));
				if(!empty($value_list)){
