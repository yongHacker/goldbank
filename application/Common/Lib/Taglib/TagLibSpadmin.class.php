...
class TagLibSpadmin extends TagLib {
    ...
    protected $tags = array(
        ...
        //门店后台模板标签
        'shoptpl' => array("attr" => "file", "close" => 0),
        //b端后台模板标签
        'businesstpl' => array("attr" => "file", "close" => 0),
        //a端后台模板标签
        'systemtpl' => array("attr" => "file", "close" => 0),
    );
     ...
     * @return array
     */
    public function _shoptpl($tag, $content) {
        $file = $tag['file'];
        $counts = count($file);
        if ($counts < 3) {
            $file_path = "Shop" .  "/" . $tag['file'];
        } else {
            $file_path = $file[0] . "/" . "Tpl" . "/" . $file[1] . "/" . $file[2];
        }
        //模板路径
     ...
     * @return array
     */
    public function _businesstpl($tag, $content) {
        $file = $tag['file'];
        $counts = count($file);
        if ($counts < 3) {
            $file_path = "Business" .  "/" . $tag['file'];
        } else {
            $file_path = $file[0] . "/" . "Tpl" . "/" . $file[1] . "/" . $file[2];
        }
        //模板路径
     ...
     * @return array
     */
    public function _systemtpl($tag, $content) {
        $file = $tag['file'];
        $counts = count($file);
        if ($counts < 3) {
            $file_path = "System" .  "/" . $tag['file'];
        } else {
            $file_path = $file[0] . "/" . "Tpl" . "/" . $file[1] . "/" . $file[2];
        }
        //模板路径
