...
class BOperateLogModel extends CommonModel{
    ...
    public function _initialize(){
        ...
        if(!$this->model_operate_table){
            ...
            $this->user_id  = session('SHOP_USER_ID')>0?get_user_id():0;
            $this->company_id  =session('SHOP_COMPANY_ID')>0?get_company_id():0;
        }
    ...
    public function _getValueDetail($table_name,$value_list){
        if(!empty($table_name)&&!empty($value_list)){
            ...
            foreach ($value_list as $key => $val){
                ...
                $field_info=D("BStatus")->getInfo($condition);
                $table_note=$this->get_table_note($table_name,$key);
                $res['field_note']=$table_note[0]['column_comment'];
                if(!empty($field_info)){
                    $is_exist=D("BStatusValue")->getInfo(array('s_id'=>$field_info['id'],'status'=>1));
                    if($is_exist){
                        $field_value_info=D("BStatusValue")->getInfo(array('s_id'=>$field_info['id'],'value'=>$val,'status'=>1));
    ...
    function is_check_ignore(){
        $ignore_arr=include APP_PATH."Shop/operate_log_ignore.php";
