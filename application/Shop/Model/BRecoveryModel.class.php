...
class BRecoveryModel extends BCommonModel{
    ...
    /*操作函数名*/
    const CHECK_FUNCTION     =  'shop/brecovery/check_post';
    //添加旧金回购
    public function add_post(){
        ...
        if($recovery_id){
            $products=D("BRecoveryProduct")->add_product($recovery_products,$recovery_id,0);
        }
        //回购单创建记录或保存记录
    ...
    public function edit_post(){
        ...
        if($recovery!==false){
            ...
            $products = D("BRecoveryProduct")->update_product($recovery_products,$recovery_order['recovery_id'],0);
        }
        //回购编辑的保存或提交的操作记录
        $record_status=$order_data['status']==0?self::COMMIT:self::SAVE;
    ...
    public function getInfo_detail(){
        $condition=array("brecovery.company_id"=>get_company_id(),"brecovery.shop_id"=>get_shop_id(),"brecovery.deleted"=>0,"brecovery.id"=>I("get.id",0,"intval"));
    ...
    public function excel_out($excel_where, $excel_field='', $excel_join='', $page = 1,$action='excel_out'){
        ...
        if($data){
            ...
            $title=array('序','单号','回购员','会员','销售总价','数量','回购时间','制单时间','状态');
