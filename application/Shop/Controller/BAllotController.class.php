...
use Shop\Model\BAllotModel;

class BAllotController extends ShopbaseController {

    public function __construct() {
        parent::__construct();
        $this->bwarehouse_model=D("BWarehouse");
        $this->ballotdetail_model=D("BAllotDetail");
        $this->ballot_model=D("BAllot");
    ...
    public function _getlist($where){
        ...
        foreach($data as $k=>$v){
           /* $data[$k]["user_nicename"]=get_user_nicename($v["creator_id"]);
            $data[$k]["check_name"]=get_user_nicename($v["check_id"]);
            $data[$k]["outbound_name"]=get_user_nicename($v["outbound_id"]);
            $data[$k]["receipt_name"]=get_user_nicename($v["receipt_id"]);*/
...
/*************************************调拨******************************************************/
    ...
    public function product_list() {
        $where=array("bproduct.status"=>2,"bproduct.warehouse_id"=>I("mystore",0,'intval'));
        A("BProduct")->product_list($where,3);
        if(empty(I("mystore"))){
    ...
    public function allot_index() {
        $condition["from_bwarehouse.shop_id|to_bwarehouse.shop_id"]=get_shop_id();
    ...
    public function allot_index_detail() {
        ...
        $condition=array("ballotdetail.allot_id"=>$data["id"]);
        $this->getList_detail($condition);
        $this->assign("allocation",$data);
        /*表单的操作记录 add by lzy 2018.5.26 start*/
        $operate_record=$this->ballot_model->getOperateRecord($data["id"]);
        $this->assign('operate_record', $operate_record);
        //表单的操作流程
        $operate_process=$this->ballot_model->getProcess($data["id"]);
    ...
    public function allot_check_post() {
        ...
        if($getdata["type"]==2){
            ...
            $record_result=D('BBillOpRecord')->addRecord(BBillOpRecordModel::ALLOT,$allot_id,BAllotModel::ALLOT_CHECK_DENY);
            /*添加表单操作记录 add by lzy 2018.5.30 end*/
        }elseif($getdata["type"]==1){
            $product_save=true;
            /*添加表单操作记录 add by lzy 2018.5.30 start*/
            $record_result=D('BBillOpRecord')->addRecord(BBillOpRecordModel::ALLOT,$allot_id,BAllotModel::ALLOT_CHECK_PASS);
            /*添加表单操作记录 add by lzy 2018.5.30 end*/
        }elseif($getdata["type"]==-2){ //驳回
            $data=array("status"=>2);
            $product_save=$this->update_product_status($allot_id,$data);
            /*添加表单操作记录 add by lzy 2018.5.30 start*/
            $record_result=D('BBillOpRecord')->addRecord(BBillOpRecordModel::ALLOT,$allot_id,BAllotModel::ALLOT_CHECK_REJECT);
            /*添加表单操作记录 add by lzy 2018.5.30 end*/
        }
        //$product_save=true;
        if ($ballot_save!==false&&$product_save!==false&&$record_result) {
            ...
            $result["status"] = 1;
            $result["msg"] = "成功";
            $result["url"] = U("BAllot/allot_check");
            $this->ajaxReturn($result);
        } else {
            M()->rollback();
            $result["status"] = 0;
    ...
    public function allot_delete() {
        ...
        $ballot_save=$this->ballot_model->update($condition,$data);
        /*添加表单操作记录 add by lzy 2018.5.30 start*/
        $record_result=D('BBillOpRecord')->addRecord(BBillOpRecordModel::ALLOT,$allot_id,BAllotModel::REVOKE);
        /*添加表单操作记录 add by lzy 2018.5.30 end*/
        $data=array("status"=>2);
        $product_save=$this->update_product_status($allot_id,$data);
        if ($ballot_save!==false&&$product_save!==false) {
       // if ($ballot_save!==false) {
            M()->commit();
            S('session_menu' . get_user_id(), null);
            $result["status"] = 1;
            $result["info"] = "成功";
            $result["url"] = U("BAllot/allot_index");
            $this->ajaxReturn($result);
        } else {
            M()->rollback();
            $result["status"] = 0;
    ...
    public function deleted() {
        $getdata = I("");
        $allot_id= I("get.id",0,'intval');
       // $data["id"] = $allot_id;
        $data["deleted"] =1;
        $condition=array("id"=>$allot_id,"status"=>array('in','-1,2,3,4,6'),"company_id"=>$this->MUser["company_id"],'creator_id'=>get_user_id());
        M()->startTrans();
        $bwallot_save=$this->ballot_model->update($condition,$data);
 ...
 /*************************************出库******************************************************/
    ...
    public function outbound_index() {
        $getdata=I("");
        $condition=array("ballot.status"=>array("gt","3"));
        $condition["from_bwarehouse.shop_id"]=get_shop_id();
    ...
    public function outbound_check_post() {
        ...
        if($getdata["type"]==2){
            ...
            $product_save=$this->update_product_status($allot_id,$data);
            /*添加表单操作记录 add by lzy 2018.5.30 start*/
            $record_result=D('BBillOpRecord')->addRecord(BBillOpRecordModel::ALLOT,$allot_id,BAllotModel::OUT_CHECK_DENY);
            /*添加表单操作记录 add by lzy 2018.5.30 end*/
        }elseif($getdata["type"]==1){
            $product_save=true;
            /*添加表单操作记录 add by lzy 2018.5.30 start*/
            $record_result=D('BBillOpRecord')->addRecord(BBillOpRecordModel::ALLOT,$allot_id,BAllotModel::OUT_CHECK_PASS);
            /*添加表单操作记录 add by lzy 2018.5.30 end*/
        }elseif($getdata["type"]==-2){
            $data=array("status"=>2);
            $product_save=$this->update_product_status($allot_id,$data);
            /*添加表单操作记录 add by lzy 2018.5.30 start*/
            $record_result=D('BBillOpRecord')->addRecord(BBillOpRecordModel::ALLOT,$allot_id,BAllotModel::OUT_CHECK_REJECT);
            /*添加表单操作记录 add by lzy 2018.5.30 end*/
        }
        //$product_save=true;
        if ($ballot_save!==false&&$product_save!==false&&$record_result) {
            ...
            $result["status"] = 1;
            $result["msg"] = "成功";
            $result["url"] = U("BAllot/outbound_check");
            $this->ajaxReturn($result);
        } else {
            M()->rollback();
            $result["status"] = 0;
...
/*************************************入库******************************************************/
    ...
    public function receipt_index() {
        $getdata=I("");
        $condition=array("ballot.status"=>array("gt",5));
    ...
    public function receipt_check_post() {
        ...
        if ($productid[0]) {
            ...
            if($getdata["type"]==2){
                $data=array("status"=>2);
                /*添加表单操作记录 add by lzy 2018.5.30 start*/
                $record_result=D('BBillOpRecord')->addRecord(BBillOpRecordModel::ALLOT,$allot_id,BAllotModel::IN_CHECK_DENY);
                /*添加表单操作记录 add by lzy 2018.5.30 end*/
            }else{
                $data=array("status"=>2,"warehouse_id"=>$ballot["to_id"]);
                /*添加表单操作记录 add by lzy 2018.5.30 start*/
                $record_result=D('BBillOpRecord')->addRecord(BBillOpRecordModel::ALLOT,$allot_id,BAllotModel::IN_CHECK_PASS);
                /*添加表单操作记录 add by lzy 2018.5.30 end*/
            }
            $product_save=$this->update_product_status($allot_id,$data);
           /* if($getdata["type"]==2){
            ...
            if ($ballot_save!==false&&$product_save!==false) {
                ...
                $result["status"] = 1;
                $result["msg"] = "成功";
                $result["url"] = U("BAllot/receipt_check");
                $this->ajaxReturn($result);
            } else {
                M()->rollback();
                $result["status"] = 0;
    ...
    public function allot_list(){
        ...
        $field='gb_b_allot.*,gb_b_warehouse.wh_name as fwh_name,wh.wh_name,COUNT(*) AS allot_num1,gb_m_users.user_nicename';
        $group='gb_b_allot.id';
