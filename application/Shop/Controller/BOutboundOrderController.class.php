...
use Shop\Model\BOutboundOrderModel;

class BOutboundOrderController extends ShopbaseController {
    ...
    public function _getlist($where){
        ...
        $condition=array("boutbound.company_id"=>$this->MUser['company_id'],"boutbound.warehouse_id"=>$wh_info['id'],"boutbound.deleted"=>0);
        if(!empty($where)){
...
/*************************************出库******************************************************/
    ...
    public function check_post() {
        ...
        if($getdata["type"]==2||$getdata["type"]==-2){//审核不通过，审核驳回，货品还原为在库状态
            ...
            $data=array();
            $data["status"]=$boutboundorder['type']==2?8:($boutboundorder['type']==4?11:12);//2,8异常出库 4，10赠送出库 5，11转料出库
            $product_save=$this->update_product_status($outbound_id,$data);
        }
