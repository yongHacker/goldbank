...
use Shop\Model\BSellModel;

class BSellController extends ShopbaseController {
    ...
    private function handleSearch(&$ex_where = NULL){
        ...
        if($getdata["search_name"]){
            $condition["bsell.order_id|bsell.sn_id|b_client.client_name|b_client.client_moblie"]=array("like","%".$getdata["search_name"]."%");
        }
        ...
        if(get_shop_id()>0){$condition['bsell.shop_id']=get_shop_id();}
    ...
    public function _getlist($where=array()){
        $getdata=I("");
        $condition=array("bsell.company_id"=>$this->MUser['company_id'],"bsell.shop_id"=>$this->MUser['shop_id'],"bsell.deleted"=>0);
        if(!empty($where)){
        ...
        $data=$this->bsell_model->alias("bsell")->getList($condition,$field,$limit,$join,$order='bsell.id desc');//die($this->bsell_model->alias("bsell")->getLastSql());
        foreach($data as $k=>$v){
            //$data[$k]["be_onsale_price"]=$this->bselldetail_model->sum('sell_price');//应销价
        }
...
/*************************************销售******************************************************/
    ...
    public function add() {
        ...
        if(empty($postdata)){
            ...
            if($info){
                ...
                $price["cut_gold_price"]=D("BBankGold")->get_price_by_bg_id($price['cut_bg_id']);
            ...
            if($shop[0]['show_common_payment']==0){
                $shopids=get_shop_id();
            }else{
                $shopids=array('in','0,'.get_shop_id());
            }
            $payment=$this->bpayment_model->getList($condition=array("deleted"=>0,"company_id"=>$this->MUser["company_id"],'shop_id'=>$shopids));
    ...
    public function add_client() {
        if(IS_POST){
            ...
            if ($status['status'] == 1) {
                $this->success($status['msg'], U("BSell/client_list"));
            } else {
    ...
    public function index_detail() {
        $data = $this->bsell_model->getInfo_detail(); // 销售单信息
        $detail_data = $this->bselldetail_model->getList_detail($data['id']); // 销售货品明细
        $sell_cut_product_list = D('BRecoveryProduct')->getList_detail(array('brecoverydetail.order_id' => $data['id'],'brecoverydetail.type' => 1)); // 销售截金明细
        $saccount_list = $this->bsell_model->getsaccount_list($data['id']); // 销售收款明细
        $sell_sub = $this->bexpencesub_model->getSublist(array('ticket_id' => $data['id'], 'ticket_type' => 1)); //销售其他费用明细
        $this->get_operate_info($data['id']);//操作流程和日志
        $sell_recovery_product_list = D('BRecoveryProduct')->getList_detail(array('brecoverydetail.order_id' => $data['id'],'brecoverydetail.type' => 4)); // 以旧换新信息
    ...
    public function revoke() {
        ...
        if ($record_result&&$bsell_save!==false&&$product_save!==false) {
            M()->commit();
    ...
    public function check_post() {
        ...
        if($getdata["type"]==2){
            ...
            //销售审核成功更新归属，存在则不更新
            D("MUserSource")->update_belong($check_info["buyer_id"]);
            // 销售审核成功更新销售明细状态为1
            $condition = array(
                'deleted' => 0,
                'sell_id' => $sell_id
        ...
        if ($recovery_detail!==false&&$record_result&&$bsell_save!==false&&$product_save!==false&&$outbound_add!==false&&$bsaccountrecord!==false) {
            M()->commit();
            S('session_menu' . get_user_id(), null);
            $result["status"] = 1;
            $result["msg"] = "成功";
            ...
            $result["status"] = 0;
            $result["msg"] = "失败";
            $result["test"] = $bsell_save."//".$product_save."//".$outbound_add;
            $this->ajaxReturn($result);
        }
    }
    //导出列表数据
    ...
    function sell_day_report(){
        $condition=array("bsell.company_id"=>$this->MUser['company_id'],"bsell.deleted"=>0,"bsell.status"=>1,"bsell.shop_id"=>$this->MUser['shop_id']);
        $getdata=I("");
        $sell_type=$getdata['sell_type'];
        if(!empty($sell_type)){
            $condition['bsell.sell_type']=$sell_type;
