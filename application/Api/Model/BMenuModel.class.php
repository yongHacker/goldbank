...
class BMenuModel extends ApiCommonModel
{
    // 自动验证
    protected $_validate = array(
        // array(验证字段,验证规则,错误提示,验证条件,附加规则,验证时间)
        array('name', 'require', '菜单名称不能为空！', 1, 'regex', ApiCommonModel:: MODEL_BOTH ),
        array('app', 'require', '应用不能为空！', 1, 'regex', ApiCommonModel:: MODEL_BOTH ),
        array('model', 'require', '控制器不能为空！', 1, 'regex', ApiCommonModel:: MODEL_BOTH ),
        array('action', 'require', '方法名称不能为空！', 1, 'regex', ApiCommonModel:: MODEL_BOTH ),
        array('app,model,action', 'checkAction', '同样的记录已经存在！', 1, 'callback', ApiCommonModel:: MODEL_INSERT   ),
        array('id,app,model,action', 'checkActionUpdate', '同样的记录已经存在！', 1, 'callback', ApiCommonModel:: MODEL_UPDATE   ),
    ...
    public function admin_menu($parentid, $with_self = false) {
        ...
        //权限检查
        if (sp_get_current_business_id() == 1) {
            //如果是超级管理员 直接通过
            return $result;
        }
        ...
        foreach ($result as $v) {
            ...
            if (preg_match('/^public_/', $action)) {
                ...
                if ( sp_auth_check(sp_get_current_business_id(),$rule_name,$relation='or',"b")){
                    $array[] = $v;
                }
