...
use Business\Model\CommonModel;
class BOperateLogModel extends CommonModel{
    ...
    public function __construct(){
        //不能去除Common  因为Business的musers调用了boperatelog而boperatelog又调用了Muser，固musers需要使用Common模块的
    ...
    public function _initialize(){
        ...
        if(!$this->model_operate_table){
            ...
            $this->user_id  = session('BUSINESS_USER_ID')>0?get_user_id():0;
            $this->company_id  =session('BUSINESS_COMPANY_ID')>0?get_company_id():0;
        }
     ...
     * @return boolean
     */
    private function _addLog($insert){
       /*if(!in_array($_SERVER['HTTP_HOST'],C('LOG_SERVER'))){
        ...
        $operate_time=time();
        $company_id = $this->company_id>0?$this->company_id:($insert['value_company_id']>0?$insert['value_company_id']:0);
    ...
    public function _getValueDetail($table_name,$value_list){
        if(!empty($table_name)&&!empty($value_list)){
            ...
            foreach ($value_list as $key => $val){
                ...
                $field_info=D("Business/BStatus")->getInfo($condition);
                $table_note=$this->get_table_note($table_name,$key);
                $res['field_note']=$table_note[0]['column_comment'];
                if(!empty($field_info)){
                    $is_exist=D("Business/BStatusValue")->getInfo(array('s_id'=>$field_info['id'],'status'=>1));
                    if($is_exist){
                        $field_value_info=D("Business/BStatusValue")->getInfo(array('s_id'=>$field_info['id'],'value'=>$val,'status'=>1));
    ...
    function is_check_ignore(){
        $ignore_arr=include APP_PATH."Business/operate_log_ignore.php";
