...
class BSellModel extends BCommonModel {
    ...
    /*操作函数名*/
    const CHECK_FUNCTION     =  'business/bsell/check_post';
    ...
    public function add_post(){
        ...
        $param = $this->get_insert_data($order, $shop_id, $order_id, $count/*, $total_onsale_price*/);//获取销售单信息
        M()->startTrans();
        // 销售单
        ...
        if($recovery_old_products != '' && count($recovery_old_products) > 0){
            $old_product_detail = D('BRecoveryProduct')->add_product($recovery_old_products,$sell_id,4,$order['recovery_wh_id']);
        }
        // 处理截金数据
        $recovery_detail = true;
        if($recovery_products != '' && count($recovery_products) > 0){
            $recovery_detail =  D('BRecoveryProduct')->add_product($recovery_products,$sell_id,1,$order['recovery_wh_id']);
        }
        ...
        if($old_product_detail!==false&&$sell_id !== false && $li !== false && $li2 !== false && $saccout_detail !== false && $recovery_detail !== false && $insert_sub !== false&&$record_result) {
            M()->commit();
            ...
            $arr['status'] = 0;
            $arr['msg'] = '添加失败！';
            $arr['test'] =$old_product_detail.'//'. $sell_id."//".$li."//".$li2."//".$saccout_detail."//".$recovery_detail."//".$insert_sub.'//'.count($recovery_products);
        }
        return $arr;
    }
    //编辑
    public function edit_post(){
        ...
        $param = $this->get_insert_data($order, $shop_id, $order_id, $count);//获取销售单信息
        M()->startTrans();
        // 销售单
        ...
        $recovery_detail = D('BRecoveryProduct')->update_product($recovery_products,$order['sell_id'],1,$order['recovery_wh_id'],$del_cut_detail);
        //$this->update_sell_cut_product_detail($recovery_products,$order['sell_id'],$shop_id,$del_cut_detail);
        $recovery_old_product_detail = D('BRecoveryProduct')->update_product($recovery_old_products,$order['sell_id'],4,$order['recovery_wh_id'],$del_old_product_detail);
        //$this->update_sell_recovery_product_detail($recovery_old_products,$order['sell_id'],$shop_id,$del_old_product_detail);
        // 处理其它费用
        ...
        if($recovery_old_product_detail!==false&&$record_result&&$sell_id !== false && $li !== false && $li2 !== false && $saccout_detail !== false && $recovery_detail !== false && $insert_sub !== false) {
            M()->commit();
            $arr['status'] = 1;
            $arr['url'] = U("BSell/index");
        } else {
    ...
    public function get_insert_data($order, $shop_id, $order_id, $count/*,$total_onsale_price*/){
        ...
        $param["recovery_wh_id"]=$order["recovery_wh_id"];
        return $param;
    }
    //批量添加收款明细
    /**
     ...
     * @return mixed
     */
    public function add_sell_cut_product_detail($recovery_products,$sell_id,$shop_id){
        $products=array();
        foreach($recovery_products as $k=>$v){
            ...
            $data['cut_gold_price']=$v['recovery_price'];
            $data['rproduct_code']=$v['rproduct_code'];
            //$data['total_weight']=$v['total_weight'];
            $data['gold_weight']=$v['total_weight'];
        ...
        $result=D("BSellCutProduct")->insertAll($products);
        return $result;
    }
//添加以旧换金明细
    /**
     ...
     * @return mixed
     */
    public function add_sell_recovery_product_detail($recovery_old_products,$sell_id,$shop_id){
        $products=array();
        foreach($recovery_old_products as $k=>$v){
            $data=array();
            $data['sell_id']=$sell_id;
            $data['company_id']=get_company_id();
            $data['shop_id']=$shop_id;
            $data['gold_price']=$v['gold_price'];
            $data['recovery_gold_price']=$v['recovery_price'];
            $data['recovery_name']=$v['recovery_name'];
            $data['rproduct_code']=$v['rproduct_code'];
            $data['total_weight']=$v['total_weight'];
            $data['gold_weight']=$v['gold_weight'];
            $data['purity']=$v['purity'];
            $data['service_fee']=$v['service_fee'];
            $data['cost_price']=$v['cost_price'];
        ...
        $result=D("BSellRecoveryProduct")->insertAll($products);
        return $result;
    }
    //批量添加收款明细
    /**
     ...
     * @return mixed
     */
    public function update_sell_cut_product_detail($recovery_products,$sell_id,$shop_id,$del_cut_detail){
        $products=array();
        foreach($recovery_products as $k=>$v){
            $data=array();
            $data['sell_id']=$sell_id;
            ...
            $data['cost_price']=$v['cost_price'];
            $data['product_id']=$v['product_id'];
            $data['id']=$v['sell_cut_product_id'];
            $products[]=$data;
        }
        $del_result=true;
        //编辑删除销售货品明细
        if(!empty($del_cut_detail)){
            $del_result=D("BSellCutProduct")->update(array('id'=>array('in',$del_cut_detail)),array('deleted'=>1));
        }
        if($recovery_products != '' && count($recovery_products) > 0){
            $result=D("BSellCutProduct")->insertAll($products,$options=array(),true);
...
//更新以旧换金明细
     ...
     * @return mixed
     */
    public function update_sell_recovery_product_detail($recovery_old_products,$sell_id,$shop_id,$del_old_product_detail){
        $products=array();
        foreach($recovery_old_products as $k=>$v){
            $data=array();
            $data['sell_id']=$sell_id;
            ...
            $data['cost_price']=$v['cost_price'];
            $data['product_code']=$v['product_code'];
            $data['id']=$v['old_product_id'];
            $products[]=$data;
        }
        $del_result=true;
        //编辑删除销售以旧换金明细
        if(!empty($del_old_product_detail)){
            $del_result=D("BSellRecoveryProduct")->update(array('id'=>array('in',$del_old_product_detail)),array('deleted'=>1));
        }
        if($recovery_old_products != '' && count($recovery_old_products) > 0){
            $result=D("BSellRecoveryProduct")->insertAll($products,$options=array(),true);
            return $result&&$del_result;
        }else{
            return $del_result;
        }
    ...
    public function getInfo_detail(){
        ...
        $field="bsell.*";
        $field.=",create_musers.employee_name user_nicename,check_musers.employee_name check_name,b_client.client_name buy_name,b_shop.shop_name,b_shop.show_common_payment";
