...
class BAllotRproductModel extends BCommonModel{
    ...
    /*操作函数名*/
    const CHECK_FUNCTION     =  'business/ballotrproduct/allot_check_post';
    const OUT_FUNCTION       =  'business/ballotrproduct/outbound_check_post';
    const IN_FUNCTION        =  'business/ballotrproduct/receipt_check_post';

    /*门店操作函数名*/
    const SHOP_OUT_FUNCTION       =  'shop/ballotrproduct/outbound_check_post';
    const SHOP_IN_FUNCTION        =  'shop/ballotrproduct/receipt_check_post';
    ...
    public function getProcess($allot_id){
        ...
        if(!empty($allot_id)){
            ...
            if(($allot_info['status']>=4)){
                ...
                //没有审核读取审核权限的员工
                $check_function=$allot_info['from_sid']>0?self::SHOP_OUT_FUNCTION:self::OUT_FUNCTION;
                $employee_name=D('BAuthAccess')->getEmployeenamesByAllotRproductid($check_function,$allot_id,1);
            ...
            if(($allot_info['status']==6||$allot_info['status']==7)){
                ...
                //没有审核读取审核权限的员工
                $check_function=$allot_info['to_sid']>0?self::SHOP_IN_FUNCTION:self::IN_FUNCTION;
                $employee_name=D('BAuthAccess')->getEmployeenamesByAllotRproductid($check_function,$allot_id,2);
