...
class BShopMenuController extends BusinessbaseController {
    ...
    public function _initialize() {
        parent::_initialize();
        $this->menu_model = D("BMenu");
        $this->auth_rule_model = D("BAuthRule");
    }

    // 后台菜单列表
    public function index() {
    	session('business_bmenu_index','BMenu/index');
        $result = $this->menu_model->where(array("is_shop"=>1))->order(array("listorder" => "ASC"))->select();
        $tree = new \Tree();
        $tree->icon = array('&nbsp;&nbsp;&nbsp;│ ', '&nbsp;&nbsp;&nbsp;├─ ', '&nbsp;&nbsp;&nbsp;└─ ');
        $tree->nbsp = '&nbsp;&nbsp;&nbsp;';
        ...
        foreach ($result as $n=> $r) {
            ...
            $result[$n]['str_manage'] = '<a href="' . U("BShopMenu/add", array("parentid" => $r['id'], "menuid" => I("get.menuid"))) . '">'.L('ADD_SUB_MENU').'</a> | <a href="' . U("BShopMenu/edit", array("id" => $r['id'], "menuid" => I("get.menuid"))) . '">'.L('EDIT').'</a> | <a class="js-ajax-delete" href="' . U("BShopMenu/delete", array("id" => $r['id'], "menuid" => I("get.menuid")) ). '">'.L('DELETE').'</a> ';
    ...
    public function lists(){
    	session('business_bmenu_index','BMenu/lists');
    	$result = $this->menu_model->where(array("is_shop"=>1))->order(array("app" => "ASC","model" => "ASC","action" => "ASC"))->select();
    		...
    		if ($this->menu_model->create()!==false) {
    			if ($this->menu_model->add()!==false) {
    				...
    				if(empty($find_rule_count)){
    					$this->auth_rule_model->add(array("name"=>$name,"module"=>$app,"type"=>"shop_url","title"=>$menu_name));//type 1-admin rule;2-user rule
    				}
    				$session_business_bmenu_index=session('business_bmenu_index');
    				$to=empty($session_business_bmenu_index)?"BMenu/index":$session_business_bmenu_index;
    				$this->_export_app_menu_default_lang($app);
    				$this->success("添加成功！", U("BShopMenu/index"));
    			} else {
    				$this->error("添加失败！");
    			}
    		} else {
    ...
    public function delete() {
        ...
        $count = $this->menu_model->where(array("parentid" => $id,"is_shop"=>1))->count();
        if ($count > 0) {
    ...
    public function edit() {
        $tree = new \Tree();
        $id = I("get.id",0,'intval');
        $rs = $this->menu_model->where(array("id" => $id,"is_shop"=>1))->find();
        $result = $this->menu_model->where(array("is_shop"=>1))->order(array("listorder" => "ASC"))->select();
        foreach ($result as $r) {
    ...
    public function edit_post() {
    	if (IS_POST) {
    	    ...
    	    $old_menu=$this->menu_model->where(array('id'=>$id,"is_shop"=>1))->find();
    		if ($this->menu_model->create()!==false) {
    			if ($this->menu_model->save() !== false) {
    				...
    				if(empty($find_rule_count)){
    				    ...
    				    if(empty($find_old_rule_id)){
    				        $this->auth_rule_model->add(array("name"=>$name,"module"=>$app,"type"=>"business_url","title"=>$menu_name));//type 1-admin rule;2-user rule
    				    }else{
    				        $this->auth_rule_model->where(array('id'=>$find_old_rule_id))->save(array("name"=>$name,"module"=>$app,"type"=>"business_url","title"=>$menu_name));//type 1-admin rule;2-user rule
    				    }
    				}else{
    					$this->auth_rule_model->where($mwhere)->save(array("name"=>$name,"module"=>$app,"type"=>"business_url","title"=>$menu_name));//type 1-admin rule;2-user rule
    				}
    				$this->_export_app_menu_default_lang($app);
    				$this->success("更新成功！");
    			} else {
    ...
    public function backup_menu(){
    	...
    	foreach ($menus as $m){
    		...
    		file_put_contents($menudir."/business_$model.php", "<?php\nreturn $menus_str;");
    ...
    private function _export_app_menu_default_lang($app){
        $menus = $this->menu_model->where(array("app"=>$app,"is_shop"=>1))->order(array("listorder"=>"ASC","app" => "ASC","model" => "ASC","action" => "ASC"))->select();
        $lang_dir=C('DEFAULT_LANG');
        $business_bmenu_lang_file_default=SITE_PATH."data/lang/$app/Lang/".$lang_dir."/business_bmenu.php";
        	
        if(!empty($business_bmenu_lang_file_default) && !file_exists_case(dirname($business_bmenu_lang_file_default))){
            mkdir(dirname($business_bmenu_lang_file_default),0777,true);
        }
        ...
        if(!empty($business_bmenu_lang_file_default)){
            file_put_contents($business_bmenu_lang_file_default, "<?php\nreturn $lang_str;");
        }
    }
    	...
    	foreach ($apps as $app){
    		if(is_dir(SPAPP.$app)){
    			...
    			foreach ($lang_dirs as $lang_dir){
    			    $business_bmenu_lang_file=SPAPP.$app."/Lang/".$lang_dir."/business_bmenu.php";
    			    $business_bmenu_lang_file_default='';
    			    if($default_lang==$lang_dir){
    			        $business_bmenu_lang_file_default=SITE_PATH."data/lang/$app/Lang/".$lang_dir."/business_bmenu.php";
    			    }
    			    
    			    if(!file_exists_case(dirname($business_bmenu_lang_file))){
    			        mkdir(dirname($business_bmenu_lang_file),0777,true);
    			    }
    				...
    				if($lang_dir!=$default_lang && is_file($business_bmenu_lang_file)){
    				     $lang=include $business_bmenu_lang_file;
    				}
    				...
    				file_put_contents($business_bmenu_lang_file, "<?php\nreturn $lang_str;");
    				if(!empty($business_bmenu_lang_file_default)){
    				    file_put_contents($business_bmenu_lang_file_default, "<?php\nreturn $lang_str;");
    				}
    			}
    ...
    private function _import_menu($menus,$parentid=0,&$error_menus=array()){
    	foreach ($menus as $menu){
    		...
    		if(!$find_rule){
    			$this->auth_rule_model->add(array("name"=>$name,"module"=>$app,"type"=>"business_url","title"=>$menu['name']));//type 1-admin rule;2-user rule
    		}else{
    			$this->auth_rule_model->where($mwhere)->save(array("module"=>$app,"type"=>"business_url","title"=>$menu['name']));//type 1-admin rule;2-user rule
    		}
    ...
    public function restore_menu(){
    	...
    	foreach ($apps as $app){
    		if(is_dir(SPAPP.$app)){
    			$menudir=SPAPP.$app."/BMenu";
    			$menu_files=sp_scan_dir($menudir."/business_*.php",null);
    			if(count($menu_files)){
    						...
    						if(!(strpos($mf, ".") === 0)){
    							...
    							foreach ($methods as $a){
    								if(!(strpos($a, "_") === 0) && !(strpos($a, "spmy_") === 0)){
    									...
    									$find_rule=$this->auth_rule_model->where($mwhere)->find();
    									if(!$find_rule){
    										$this->auth_rule_model->add(array("name"=>$name,"module"=>$g,"type"=>"business_url","title"=>""));//type 1-admin rule;2-user rule
    									}
    								}
    							}
    						}
