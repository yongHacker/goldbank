...
class BusinessbaseController extends AppframeController {
	protected $MUser,$pagenum,$users_model,$default_password;
	...
	function _initialize(){
       ...
       $session_admin_id=session('BUSINESS_USER_ID');
    	if(!empty($session_admin_id)){
    		$users_obj= M("MUsers");
    		$user=$users_obj->where(array('id'=>$session_admin_id))->find();
			$this->MUser=$user;
			$this->MUser["company_id"]=get_company_id();
			$company=D("Business/BCompany")->getInfo(array("company_id"=>get_company_id()));
			...
			if(IS_AJAX){
				$this->error("您还没有登录！",U("business/public/login"));
			}else{
				//header("Location:".U("admin/public/login"));
				echo("<script>window.parent.location.href='".U("business/public/login")."'</script>");
				exit();
			}
	...
	private function check_access($uid){
		...
		$no_need_check_rules=array("BusinessIndexindex","BusinessMainindex");
    ...
    private function load_app_admin_menu_lang(){
		...
		if($default_lang!=$langSet){
			...
			$admin_menu_lang_file2=SPAPP.MODULE_NAME."/Lang/".$langSet."/system_amenu.php";
		}else{
			...
			$admin_menu_lang_file2=SITE_PATH."data/lang/".MODULE_NAME."/Lang/$langSet/system_amenu.php";
	...
	public function b_show_status($table){
		...
		if(!empty($status_list)){
			foreach($status_list as $key=>$val){
				...
				$field=$val['field'];
				$tpl=array();
				$value_list=$status_model->get_value_list(array('s_id'=>$s_id),null, 'FLOOR(bstatusvalue.value) ASC');
				if(!empty($value_list)){
