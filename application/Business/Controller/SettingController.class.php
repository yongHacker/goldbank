...
class SettingController extends BusinessbaseController{
	
	protected $options_model,$model_user,$model_option,$model_warehouse;
	
	public function _initialize() {
		parent::_initialize();
		$this->options_model = D("Common/Options");//加载系统网站配置，暂时不删除
		$this->model_user=D('MUsers');
		$this->model_option=D('BOptions');
		$this->model_warehouse=D('BWarehouse');
	}
	public function initialize_company_data(){
	    if(empty(I('post.'))){
	        $this->display();
	    }else{
	        $password=I('post.password');
	        $verify_result=$this->model_user->verifyPassword(get_user_id(),$password);
	        if(!$verify_result){
	            $this->error('密码错误！');
	        }else{
	            $this->model_option->initializeCompanyData();
	            $this->success('操作成功！');
	        }
	    }
	}
