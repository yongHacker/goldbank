...
use Business\Model\BRecoveryModel;
use Business\Model\BSellModel;

class BRecoveryController extends BusinessbaseController {
    ...
    public function rproduct_list(){
        ...
        if(!empty($search_name)){
            $where['bproduct.recovery_name|bproduct.rproduct_code'] = array('like', '%' . $search_name . '%');
        }

        $count = $this->brecoverydetail_model->alias("bproduct")->countList($where);
        $page = $this->page($count, $this->pagenum);
        $limit = $page->firstRow . "," . $page->listRows;
        $data = $this->brecoverydetail_model->alias("bproduct")->getList($where, $field = 'bproduct.*', $limit, $join = null, $order ='bproduct.id desc');
    ...
    public function rproduct_export(){
        ...
        $this->brecoverydetail_model->excel_out($where);
    }

    //获取回购列表数据
    public function _getlist($where=array()){
        $shop_condition=array("company_id"=>$this->MUser['company_id'],'deleted'=>0);
        $shop_list=$this->bshop_model->getList($shop_condition);
       ...
       // $field.=",create_musers.user_nicename,check_musers.user_nicename check_name,recovery_musers.user_nicename recovery_name";
        ...
        $join.=' left join '.DB_PRE.'b_recovery_product brproduct on brproduct.order_id=brecovery.id and brproduct.type=0';
        ...
        foreach ($status_list as $key => $value) {
            $status[$value['value']] = $value['comment'];
        }
...
/*************************************回购******************************************************/
    ...
    public function product_list() {
        ...
        A("Business/BProduct")->product_list($where);
    ...
    public function add_client(){
        if(IS_POST){
            ...
            if ($status['status'] == 1) {
                $this->success($status['msg'], U("BRecovery/client_list",array('shop_id'=>$shop_id)));
            } else {
    ...
    public function check_post() {
        $getdata = I("");
        $recovery_id = I("post.id",0,'intval');
        $data["id"] = $recovery_id;//回购单id
        $data["status"] = $getdata["type"];//-1 驳回为-2状态，1审核通过，2审核不通过
        $data["check_time"] = time();//审核时间
        $data["check_id"] = $this->MUser['id'];//审核人的用户id
        $data["check_memo"] = I('post.check_memo');//审核备注
        $condition = array(
            "id"=> $recovery_id,
            "status"=> 0,
            "company_id"=> $this->MUser["company_id"]
        );
        $check_info=$this->brecovery_model->getInfo($condition);//查询该回归单为当前商户的提交状态才可以继续操作
        if(empty($check_info)){
            $result["status"] = 0;
            $result["info"] = "操作失败";
            $this->ajaxReturn($result);
        ...
        M()->startTrans();//开启事务
        $brecovery_save=$this->brecovery_model->update($condition,$data);// 更改 recovery_product 状态
        if($brecovery_save !== false){
            ...
            if($getdata["type"]>0) {
                ...
                $rs=$this->brecoverydetail_model->update_status($recovery_id,0, $status);
            }
        }
        //销售审核的操作记录
        ...
        if ($record_result&&$brecovery_save !== false && $rs !== false) {
            M()->commit();
            S('session_menu' . get_user_id(), null);
            $result["status"] = 1;
            $result["info"] = "操作失败";
    ...
    function setting(){
        if (IS_POST) {
            ...
            $option_value['recovery_bgt_id']=$postdata["recovery_bgt_id"];
            $option_value['bgt_id']=$postdata["bgt_id"];
            $option_value['cut_bgt_id']=$postdata["cut_bgt_id"];
