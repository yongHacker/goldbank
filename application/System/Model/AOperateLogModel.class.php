...
class AOperateLogModel extends CommonModel{
    protected $model_operate_table;
    protected $connection;
    protected $model_user,$table_suf,$user_id;
    ...
    public function _initialize(){
        if (!OPEN_LOG) {
            $this->model_operate_table = M('AOperateLog');
            return true;
        }
        ...
        if(!$this->model_operate_table){
            ...
            $this->model_operate_table=M('log'.$table_suf,'a_operate_',C('DB_OPERATE_LOG'),1);
        }
    }
    ...
    public function _addLog($insert){
       /*if(!in_array($_SERVER['HTTP_HOST'],C('LOG_SERVER'))){
        ...
        $user_info=D(self::MUSERS)->getInfo(array('id'=>$user_id));
        $operate_ip=get_client_ip();
        $operate_time=time();
        $insert['user_id']=empty($user_info)?0:$user_id;
    ...
    function is_check_ignore(){
        ...
        if(!empty($m)){
            ...
            //$a=$m['ACTION_NAME'][strtolower(ACTION_NAME)];
