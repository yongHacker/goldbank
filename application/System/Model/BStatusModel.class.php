...
class BStatusModel extends ACommonModel{
    //增加status记录
    ...
    //获得一个状态的所有信息;
    public function _getStatusInfo($condition){
        $status_info=$this->get_status_info($condition);
        if(!empty($status_info)){
            $status_id=$status_info['id'];
            $value_list=$this->get_value_list(array('s_id'=>$status_id));
    ...
    //获取多个状态的所有信息
    public function _getStatusList($condition,$limit=null,$order=null){
    ...
    //列表形式获取单个status信息
    public function getStatusInfo($condition){
        $status_info=$this->get_status_info($condition);
        if(!empty($status_info)){
            $status_id=$status_info['id'];
            $value_list=$this->get_value_list(array('s_id'=>$status_id));
