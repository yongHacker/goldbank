...
class UsdcnyPriceModel extends ACommonModel{
	protected $kjjb_market,$kjjb,$table_pre;
	protected $connect;
	public function __construct(){
		$this->connect=C("JJH_DB");
		$this->table_pre="usdcny_price_";
		...
		if(!$this->kjjb_market){
			$sql="CREATE TABLE IF NOT EXISTS `".$tablename."` (
			 `id` int(11) NOT NULL AUTO_INCREMENT,
			  ...
			  PRIMARY KEY (`id`)
			) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8mb4 COMMENT='美元人民币汇率表';";
	...
	public function insert($data){
		$xhhj_key='JO_56382';
		$o_data=$data['o_data'][$xhhj_key];
		//现货黄金
		$data=array();
		$data['price']=$o_data['q63'];
		$data['prevclose']=$o_data['q2'];
		$data['open']=$o_data['q1'];
		$data['hight']=$o_data['q3'];
		$data['low']=$o_data['q4'];
		$data['buy_price']=$o_data['q5'];
		$data['sell_price']=$o_data['q6'];
		$data['pricedown_price']=$o_data['q70'];
		$data['pricedown_precent']=$o_data['q80'];
		$data['status']=$o_data['status'];
		$data['time']=$o_data['time']/1000;
				...
				foreach ($title_arr as $v) {
					if($v){
						...
						if($v=='time'||$v=='create_time'){
							$row[$v] = date("Y-m-d H:i",$value[$v]);
						}
						if($v=='status'){
							$row[$v] = iconv('utf-8','gbk',$value[$v]==100?'交易中':'闭市');
						}
					}
				}
				$content .= implode(",", $row) . "\n";
			}
			header("Content-Disposition: attachment; filename=美元人民币汇率.csv");
