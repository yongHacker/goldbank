...
class XauusdCnyPriceModel extends ACommonModel{
	protected $kjjb_market,$kjjb,$table_pre;
	protected $connect;
	public function __construct(){
		$this->connect=C("JJH_DB");
		$this->table_pre="xauusd_cny_price_";
		...
		if(!$this->kjjb_market){
			$sql="CREATE TABLE IF NOT EXISTS `".$tablename."` (
			 `id` int(11) NOT NULL AUTO_INCREMENT,
			  ...
			  `exchange_rate` decimal(12,4) NOT NULL COMMENT '美元人民币汇率',
			  `exchange_price` decimal(12,4) NOT NULL COMMENT '汇率后的价格',
			  ...
			  PRIMARY KEY (`id`)
			) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8mb4 COMMENT='现货黄金cny行情表';";
	...
	public function insert($data){
		...
		//现货黄金
		$data=array();
		$data['price']=$g_data[$xhhj_key]['q63'];
		$data['exchange_rate']=$o_data[$rate_key]['q63'];
		$data['exchange_price']=$o_data[$xhhj_key]['q63'];
		$data['prevclose']=$g_data[$xhhj_key]['q2'];
		$data['open']=$g_data[$xhhj_key]['q1'];
		$data['hight']=$g_data[$xhhj_key]['q3'];
		$data['low']=$g_data[$xhhj_key]['q4'];
		$data['buy_price']=$g_data[$xhhj_key]['q5'];
		$data['sell_price']=$g_data[$xhhj_key]['q6'];
		$data['pricedown_price']=$g_data[$xhhj_key]['q70'];
		$data['pricedown_precent']=$g_data[$xhhj_key]['q80'];
		$data['status']=$g_data[$xhhj_key]['status'];
		$data['time']=$g_data[$xhhj_key]['time']/1000;
...
//查询相关表，并创建或修改视图
	...
	public function export_csv($where, $field, $join, $page = 1){
		...
		if($data_list){
			...
			header("Content-Disposition: attachment; filename=现货黄金行情.csv");
