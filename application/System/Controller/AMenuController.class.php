...
class AMenuController extends SystembaseController {
    ...
    public function _initialize() {
        parent::_initialize();
        $this->menu_model = D("AMenu");
        $this->auth_rule_model = D("AAuthRule");
    }

    // 后台菜单列表
    public function index() {
    	session('system_amenu_index','AMenu/index');
        ...
        foreach ($result as $n=> $r) {
            ...
            $result[$n]['str_manage'] = '<a href="' . U("AMenu/add", array("parentid" => $r['id'], "menuid" => I("get.menuid"))) . '">'.L('ADD_SUB_MENU').'</a> | <a href="' . U("AMenu/edit", array("id" => $r['id'], "menuid" => I("get.menuid"))) . '">'.L('EDIT').'</a> | <a class="js-ajax-delete" href="' . U("AMenu/delete", array("id" => $r['id'], "menuid" => I("get.menuid")) ). '">'.L('DELETE').'</a> ';
    ...
    public function lists(){
    	session('system_amenu_index','AMenu/lists');
    	...
    	if (IS_POST) {
    		if ($this->menu_model->create()!==false) {
    			if ($this->menu_model->add()!==false) {
    				...
    				$find_rule_count=$this->auth_rule_model->countList($where);
    				if(empty($find_rule_count)){
    					$this->auth_rule_model->insert(array("name"=>$name,"module"=>$app,"type"=>"system_url","title"=>$menu_name));//type 1-admin rule;2-user rule
    				}
    				$session_system_amenu_index=session('system_amenu_index');
    				$to=empty($session_system_amenu_index)?"BMenu/index":$session_system_amenu_index;
    			...
    			if ($this->menu_model->save() !== false) {
    				...
    				if(empty($find_rule_count)){
    				    ...
    				    if(empty($find_old_rule_id)){
    				        $this->auth_rule_model->add(array("name"=>$name,"module"=>$app,"type"=>"system_url","title"=>$menu_name));//type 1-admin rule;2-user rule
    				    }else{
    				        $this->auth_rule_model->where(array('id'=>$find_old_rule_id))->save(array("name"=>$name,"module"=>$app,"type"=>"system_url","title"=>$menu_name));//type 1-admin rule;2-user rule
    				    }
    				}else{
    					$this->auth_rule_model->where($mwhere)->save(array("name"=>$name,"module"=>$app,"type"=>"system_url","title"=>$menu_name));//type 1-admin rule;2-user rule
    				}
    				$this->_export_app_menu_default_lang($app);
    				$this->success("更新成功！");
    			} else {
    ...
    public function backup_menu(){
    	...
    	foreach ($menus as $m){
    		...
    		file_put_contents($menudir."/system_$model.php", "<?php\nreturn $menus_str;");
    ...
    private function _export_app_menu_default_lang($app){
        ...
        $system_amenu_lang_file_default=SITE_PATH."data/lang/$app/Lang/".$lang_dir."/system_amenu.php";
        	
        if(!empty($system_amenu_lang_file_default) && !file_exists_case(dirname($system_amenu_lang_file_default))){
            mkdir(dirname($system_amenu_lang_file_default),0777,true);
        }
        ...
        if(!empty($system_amenu_lang_file_default)){
            file_put_contents($system_amenu_lang_file_default, "<?php\nreturn $lang_str;");
        }
    }
    	...
    	foreach ($apps as $app){
    		if(is_dir(SPAPP.$app)){
    			...
    			foreach ($lang_dirs as $lang_dir){
    			    $system_amenu_lang_file=SPAPP.$app."/Lang/".$lang_dir."/system_amenu.php";
    			    $system_amenu_lang_file_default='';
    			    if($default_lang==$lang_dir){
    			        $system_amenu_lang_file_default=SITE_PATH."data/lang/$app/Lang/".$lang_dir."/system_amenu.php";
    			    }
    			    
    			    if(!file_exists_case(dirname($system_amenu_lang_file))){
    			        mkdir(dirname($system_amenu_lang_file),0777,true);
    			    }
    				...
    				if($lang_dir!=$default_lang && is_file($system_amenu_lang_file)){
    				     $lang=include $system_amenu_lang_file;
    				}
    				...
    				file_put_contents($system_amenu_lang_file, "<?php\nreturn $lang_str;");
    				if(!empty($system_amenu_lang_file_default)){
    				    file_put_contents($system_amenu_lang_file_default, "<?php\nreturn $lang_str;");
    				}
    			}
    ...
    private function _import_menu($menus,$parentid=0,&$error_menus=array()){
    	foreach ($menus as $menu){
    		...
    		if(!$find_rule){
    			$this->auth_rule_model->add(array("name"=>$name,"module"=>$app,"type"=>"system_url","title"=>$menu['name']));//type 1-admin rule;2-user rule
    		}else{
    			$this->auth_rule_model->where($mwhere)->save(array("module"=>$app,"type"=>"system_url","title"=>$menu['name']));//type 1-admin rule;2-user rule
    		}
    ...
    public function restore_menu(){
    	...
    	foreach ($apps as $app){
    		if(is_dir(SPAPP.$app)){
    			$menudir=SPAPP.$app."/AMenu";
    			$menu_files=sp_scan_dir($menudir."/system_*.php",null);
    			if(count($menu_files)){
    ...
    private function _generate_submenu(&$rootmenu,$m){
    	...
    	if(count($rm)){
    		...
    		$count=count($rm);
    		for($i=0;$i<$count;$i++){
    			$this->_generate_suamenu($rm[$i],$rm[$i]);
    						...
    						if(!(strpos($mf, ".") === 0)){
    							...
    							foreach ($methods as $a){
    								if(!(strpos($a, "_") === 0) && !(strpos($a, "spmy_") === 0)){
    									...
    									$find_rule=$this->auth_rule_model->where($mwhere)->find();
    									if(!$find_rule){
    										$this->auth_rule_model->add(array("name"=>$name,"module"=>$g,"type"=>"system_url","title"=>""));//type 1-admin rule;2-user rule
    									}
    								}
    							}
    						}
