...
class BShopMenuController extends SystembaseController {
    			...
    			if ($this->menu_model->save() !== false) {
    				...
    				if(empty($find_rule_count)){
    				    ...
    				    if(empty($find_old_rule_id)){
    				        ...
    				        $this->auth_rule_model->where(array('id'=>$find_old_rule_id))->save(array("name"=>$name,"module"=>$app,"type"=>"shop_url","title"=>$menu_name));//type 1-admin rule;2-user rule
    				    }
    				}else{
    					$this->auth_rule_model->where($mwhere)->save(array("name"=>$name,"module"=>$app,"type"=>"shop_url","title"=>$menu_name));//type 1-admin rule;2-user rule
    				}
    				$this->_export_app_menu_default_lang($app);
    				$this->success("更新成功！");
    			} else {
    ...
    private function _import_menu($menus,$parentid=0,&$error_menus=array()){
    	foreach ($menus as $menu){
    		...
    		if(!$find_rule){
    			$this->auth_rule_model->add(array("name"=>$name,"module"=>$app,"type"=>"shop_url","title"=>$menu['name']));//type 1-admin rule;2-user rule
    		}else{
    			$this->auth_rule_model->where($mwhere)->save(array("module"=>$app,"type"=>"shop_url","title"=>$menu['name']));//type 1-admin rule;2-user rule
    		}
    						...
    						if(!(strpos($mf, ".") === 0)){
    							...
    							foreach ($methods as $a){
    								if(!(strpos($a, "_") === 0) && !(strpos($a, "spmy_") === 0)){
    									...
    									$find_rule=$this->auth_rule_model->where($mwhere)->find();
    									if(!$find_rule){
    										$this->auth_rule_model->add(array("name"=>$name,"module"=>$g,"type"=>"shop_url","title"=>""));//type 1-admin rule;2-user rule
    									}
    								}
    							}
    						}
