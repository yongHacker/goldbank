...
var API_URL=ROOT_URL+"/index.php?g=Kunjinjubao";
var SYSTEM_API_URL=ROOT_URL+"/index.php?g=System";
;(function () {
...
//生成一个可控长度的随机字符串充当当前页面ID
function location_id(length){
    length = (length == undefined || length == 0) ? 10 : length;
...
/**
 * 通用表格排序
 * @param  string           tbody       tbody带前缀的id或class 或tbody对象
 * @param  string|object    operate_obj 操作对象 或 操作对象带前缀的class或id
 */
function rule_table_order(tbody, operate_obj) {
    if (typeof(tbody) != 'undefined' && tbody != '') {
        if (typeof(operate_obj) != 'object') {
            tbody = $(tbody);
        }
    } else if (typeof(operate_obj) != 'undefined' && operate_obj != '') {
        if (typeof(operate_obj) == 'object') {
            tbody = operate_obj.closest('tbody');
        } else {
            tbody = $(operate_obj).closest('tbody');
        }
    }
    if (typeof(tbody)  != 'undefined') {
        ...
        for (var i = 0; i < len; i++) {
            tbody.find('tr:eq(' + i + ') td:first').text((i + 1));
        }
    }
}


// 预先定义tbody->tr中的data或者属性 eg : <tr data-productid="1"></tr> 或者 <tr productid="1"></tr>
// 用法 ： refresh_tr_data($('.product'), 'productid', 'product_id' + location_id, 'last_product');
// 用法 ： refresh_tr_data('.product', 'productid', 'product_id' + location_id, 'last_product');
/**
 * 获取tbody->tr中的参数值序列，刷新localStorage并返回
 * @param  string|object    tbody       tbody带前缀的id或class 或tbody对象
 * @param  string           param       要获取的参数名称
 * @param  string           storage_id  localStorage参数下标
 * @param  string           exclude_id  被排除的行id
 * @return string           param_seq   值序列
 */
function refresh_tr_seq(tbody, param, storage_id, exclude_id)
{
    var param_source = 'data';
    if (typeof(tbody) != 'undefined' && tbody != '') {
        if (typeof(operate_obj) != 'object') {
            tbody = $(tbody);
        }
    ...
    if (tr.length != 0 && typeof(tr.eq(0).data(param)) == 'undefined') {
        param_source = 'attr';
    }

    var param_seq = '';
    tr.each(function() {
        if (typeof(exclude_id) != 'undefined' && $(this).attr('id') == exclude_id) {
            return false;
        }

        if (param_source == 'data') {
            var param_value = $(this).data(param);
        } else {
            var param_value = $(this).attr(param);
        }
        param_seq += (String((typeof(param_value) == 'undefined' ? '' : param_value)) + ',');
    });
    param_seq = param_seq.substring(0, param_seq.length - 1);
    localStorage.setItem(storage_id, param_seq);
    return param_seq;
}

/**
 * 检查localStorage中的参数值序列是否存在value值，与refresh_tr_seq搭配使用
 * @param  string storage_id localStorage参数下标
 * @param  string value      对比的值
 * @return boolean           false-不存在 true-存在
 */
function check_record_exist(storage_id, value) {
    var storage_value = localStorage.getItem(storage_id);
    storage_value = (storage_value == null) ? new Array() : storage_value.split(',');
    if (storage_value.indexOf(String(value)) == -1) {
        return false;
    }
    return true;
}
...
// 数字输入框格式化
// eg:number_formatting($(this));
function number_formatting(obj) { 
    ...
    if (typeof(step) == 'undefined' || step == '') {
        ...
        step = (step % 1 == 0) ? 0 : get_float_local(step);
    }
    // 控制小数点位数
    if (get_float_local(value) > step) {
        obj.val(parseFloat(value).toFixed(step));
    }
}
// 获取浮点数小数位数
function get_float_local(float) {
    float = float.toString();
    var len = float.length;
    var local =  float.indexOf('.') + 1;
    return len - local;
}
