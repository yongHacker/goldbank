<?php
namespace Org\WechatPhpSdk;

use Org\WechatPhpSdk\Utils\HttpCurl;
use Org\WechatPhpSdk\Utils\Error;
use Org\WechatPhpSdk\Utils\SHA1;
use Org\WechatPhpSdk\Utils\Xml;
...
class Api 
{
    ...
    public function get_jsapi_config($url = '', $type = '', $jsonp_callback = 'callback')
    {
        /* $cache = S(array(
                 'type' => 'Memcached',
                 'prefix' => C('MEM_PRE'), "expire" => 5400)
         );
         if (I('get.debug') == '1') {
             $cache->set('signPackage', '');
         }*/
        $jsapi_config = "";// $cache->get('signPackage');
        if (!empty($jsapi_config)) {
            return $jsapi_config;
        } else {
            ...
            //@error_log($jsapi_ticket . "//" . $nonce_str . "//" . $timestamp . "//" . $url);
            ...
            // $cache->set('signPackage', $jsapi_config);
            return $jsapi_config;
        }
    ...
    public function get_jsapi_ticket()
    {
        ...
        if ($this->get_jsapi_ticket_diy !== false) {
            ...
            //@error_log("jsapi_ticket2:" . $ticket);
        } else {
    ...
    public function is_follow($openid) {
        // if (empty($_COOKIE["is_follow"])) {
            $access_token = $this->get_access_token();
            $url = self::API_DOMAIN . 'cgi-bin/user/info?access_token=' . $access_token . '&openid=' . $openid;
            $res = HttpCurl::get($url, 'json');
            // 异常处理: 获取时网络错误
            if ($res === false) {
                @error_log("网络错误！");
            ...
            if (isset($res->subscribe) && $res->subscribe == 1) {
                setcookie("is_follow", "1", time() + 7200);
                @error_log('is_follow:' . $_COOKIE["is_follow"]);
                // return array(null, $res);
                return 1;
            } else {
                setcookie("is_follow", "2", time() + 7200);
                @error_log('is_follow:' . $_COOKIE["is_follow"]);
                return 2;
                //return array($res, null);
            }
     ...
     * @param $template_id  公众平台的的我的模板的模板id
     * @param $url          模板信息的跳转链接
     * @param $data         模板信息
    ...
    public function doSend($touser, $template_id, $url, $data, $topcolor = '#7B68EE') {
        ...
        $res = HttpCurl::post($url, $json_template, 'json');
        // 异常处理: 获取access_token网络错误
        if ($res === false) {
            return Error::code('ERR_GET');
        }
