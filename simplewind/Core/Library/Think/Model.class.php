...
class Model {
    ...
    public function __construct($name='',$tablePrefix='',$connection='',$linkNum='0') {
        ...
        // 获取数据库操作对象
        // 当前模型有独立的数据库连接信息
        $this->db($linkNum,empty($this->connection)?$connection:$this->connection,true);
    }
    ...
    public function add($data='',$options=array(),$replace=false) {
        ...
        $before_insert_result=$this->_before_insert($data,$options);
        if(false === $before_insert_result) {
            return false;
        }
        // 写入数据到数据库
        ...
        if(false !== $result && is_numeric($result)) {
            ...
            $this->_delinsert_write($before_insert_result);
        }
        return $result;
    }
    // 插入数据前的回调方法
    ...
    public function addAll($dataList,$options=array(),$replace=false){
        ...
        // 分析表达式
        $options =  $this->_parseOptions($options);
        $before_addAll=$this->_before_addAll($dataList,$options);
        if(false === $before_addAll) {
            return false;
        }
        // 写入数据到数据库
        ...
        if(false !== $result ) {
            $insertId   =   $this->getLastInsID();
            if($insertId) {
                if(false === $this->_after_addAll($dataList,$options)) {
                    return false;
                }
                return $insertId;
            }else{
                $this->_deladdAll_write($before_addAll);
            }
            if(false === $this->_after_addAll($dataList,$options)) {
                return false;
            }
    ...
    // 批量插入失败后回调方法
    protected function _deladdAll_write($str) {}
    // 插入成功后的回调方法
    protected function _after_addAll($data,$options) {}
    /**
     * 通过Select方式添加记录
     * @access public
    ...
    public function selectAdd($fields='',$table='',$options=array()) {
        ...
        $options =  $this->_parseOptions($options);
        // 写入数据到数据库之前的操作
        $before_selectAdd=$this->_before_selectAdd($fields?:$options['field'],$table?:$this->getTableName(),$options);
        if(false === $before_selectAdd) {
            return false;
        }
        // 写入数据到数据库
        if(false === $result = $this->db->selectInsert($fields?:$options['field'],$table?:$this->getTableName(),$options)){
            $this->_delselectAll_write($before_selectAdd);
            // 数据库插入操作失败
    ...
    public function save($data='',$options=array()) {
        ...
        $before_update_result=$this->_before_update($data,$options);
        if(false === $before_update_result) {
            return false;
        }
        ...
        if($result===0){
            $this->_delupdate_write($before_update_result);
        }
        return $result;
    }
    // 更新数据前的回调方法
    ...
    //更新数据失败回调方法
    protected function _delupdate_write($str) {}
    // 更新成功后的回调方法
    ...
    public function delete($options=array()) {
        ...
        $before_delete_result=$this->_before_delete($options);
        if(false === $before_delete_result) {
            return false;
        }
    ...
    // 删除数据失败回调方法
    protected function _deldelete_write($str) {}
    // 删除成功后的回调方法
